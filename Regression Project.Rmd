...
First, we did some simple summaries of the data.  As you can see, there are equal numbers of manual and automatic cars.
...
However, it seems that automatic cars are generally heavier,have more cylinders, and have more horsepower, and more displacement, which can all be factors in causing worse mileage. (Look at pairs function output and histogram in appendix).
```{r}
...
We compare several models below.  Our analysis shows that we can account for weight, manual/automatic, and displacement, but other variables seem unnecessary. Note that fit2 doesn't change the coefficient values for data$am too much.
...
We calculate some residuals. The plot shows residuals are good for model fit, there's no discernible pattern, and taking regression of resids shows no slope to residuals.
```{r}
plot(data$wt,resid(fit))
lm(data$wt~resid(fit))##
```

Fit shows a slight downward slope to resids, which indicates this model doesn't fit the data well.  This is shown below.
```{r}
